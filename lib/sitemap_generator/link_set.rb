...
module SitemapGenerator
  class LinkSet
    ...
    attr_accessor :verbose, :yahoo_app_id, :include_root, :include_index, :sitemaps_host, :adapter, :search_engines
    ...
    def ping_search_engines(*args)
      ...
      sitemap_index_url = CGI.escape(args.shift || sitemap_index.location.url)

      require 'open-uri'
      require 'timeout'

      puts "\n" if verbose
      search_engines.merge(opts[:search_engines]).each do |engine, link|
        link = link % sitemap_index_url
        begin
          Timeout::timeout(10) {
            open(link)
          }
